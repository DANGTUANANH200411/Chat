        ...
        <Row className='preview-chat-item' onClick={onClick}>
            ...
            <Col span={19}>
                <Row>
                    ...
                    <Col span={6}>
                        <ChatRoomMenu />
                        <span className='preview-chat-item-time text-secondary text-small'>{timeFromNow('20240706113102')}</span>
                    </Col>
                </Row>
                {previewMsg && (
                    <div className='text-secondary text-ellipsis text-small'>{`${users.get(previewMsg.sender)?.userName ?? ''}: ${previewMsg.message}`}</div>
                )}
            </Col>
        </Row>
    );
